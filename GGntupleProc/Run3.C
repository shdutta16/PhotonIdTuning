...
void Run3::Loop(string outputfile, int ntotEvent, int nPrintEvent, double evtWeight)
{
  TStopwatch sw;
  sw.Start();
  ...
  float Vtx,Vty,Vtz,MCVtx,MCVty,MCVtz,gedPhR9,gedPhEta,gedPhSCEta,gedPhPt,gedPhPhi,gedPhoIso,gedChgIso,gedNeuIso,gedPhTower,gedPhSieie,Rh,gedGenPt,gedPhweightXS,gedPhIDMVA,gedPhEcalPFClusIso,gedPhHcalPFClusIso; 
  ...
  t1->Branch("gedPhEcalPFClusIso",&gedPhEcalPFClusIso,"gedPhEcalPFClusIso/F");
  t1->Branch("gedPhHcalPFClusIso",&gedPhHcalPFClusIso,"gedPhHcalPFClusIso/F");
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      for(unsigned int ipho = 0; ipho < (*phoEt).size(); ipho++){
	...
	gedPhIDMVA = (*phoIDMVA)[ipho];
        gedPhEcalPFClusIso = (*phoEcalPFClusterIso)[ipho];
        gedPhHcalPFClusIso = (*phoHcalPFClusterIso)[ipho];
